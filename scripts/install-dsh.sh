#!/usr/bin/env bash
# ============================================================================
# install-dsh.sh — 把 dsh-research-starter 装入 DSH（DeepSeek Harness）
#
# 装入内容：
#   1. agent preset：presets/research/ → $DSH_HOME/.agent-presets/research/
#      （agent.cordis.yml + preset.yml；DSH 每次读 roster 时重新发现，无需重启）
#   2. 随包 skills：presets/research/skills/（vault-protocol/reporting/collecting）
#      随 preset 一起复制、由 preset 的 skill-filesystem 行以 preset 层（scoped）
#      注册——研究模式内 shadow 全局同名技能，模式外不影响。★ 因此本脚本
...
#   - 安装后新开一个 DSH 会话（选择"研究模式"preset）即可生效；
#     正在运行的会话保持旧 persona，不需要也不应该重启 3082。
# ============================================================================
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PRESET_SRC="$ROOT/presets/research"
...
echo "== dsh-research-starter 安装 (mode=$MODE) =="
...
echo "== 1/1 agent preset（含 skills/）→ $DSH_HOME/.agent-presets/research =="
PRESET_DST="$DSH_HOME/.agent-presets/research"
if [ -e "$PRESET_DST" ]; then
    mv "$PRESET_DST" "$PRESET_DST.bak-$TS"
...
echo ""
echo "✅ 安装完成。"
echo "  - 新会话选择『研究模式』preset 即生效（DSH 每次读 roster 重新发现，无需重启）。"
echo "  - 随包 skills（vault-protocol/reporting/collecting）以 preset 层注册，研究模式内 shadow 全局；"
